...
func (r *regionFailureHandler) handleError(ctx context.Context, errInfo regionErrorInfo) error {
	...
	retryPriorityIntent := retryScanIntent(errInfo.regionInfo)
	...
	case *eventError:
		...
		if notLeader := innerErr.GetNotLeader(); notLeader != nil {
			...
			r.client.scheduleRegionRequest(ctx, errInfo.regionInfo, retryPriorityIntent)
			return nil
		}
		if innerErr.GetEpochNotMatch() != nil {
			metricFeedEpochNotMatchCounter.Inc()
			r.client.scheduleRangeRequest(ctx, errInfo.span, errInfo.subscribedSpan, errInfo.filterLoop, retryPriorityIntent)
			return nil
		}
