...
func (r *regionFailureHandler) handleError(ctx context.Context, errInfo regionErrorInfo) error {
	...
	inheritedDecision := scanPriorityDecisionFromRegion(errInfo.regionInfo)
	...
	case *eventError:
		...
		if notLeader := innerErr.GetNotLeader(); notLeader != nil {
			...
			r.client.scheduleRegionRequest(ctx, errInfo.regionInfo, inheritedDecision)
			return nil
		}
		if innerErr.GetEpochNotMatch() != nil {
			metricFeedEpochNotMatchCounter.Inc()
			r.client.scheduleRangeRequest(ctx, errInfo.span, errInfo.subscribedSpan, errInfo.filterLoop, inheritedDecision)
			return nil
		}
